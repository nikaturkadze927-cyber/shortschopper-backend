...
async def cut_shorts(request: LinkRequest, current_user: str = Depends(get_current_user)):
    ...
    try:
        # იუთუბის ლინკიდან ვიღებთ მხოლოდ ვიდეოს ID-ს
        video_id = url.split("v=")[-1].split("&")[0].split("/")[-1]
        
        # ვიყენებთ Invidious-ის ერთ-ერთ მუშა სერვერს ვიდეოს პირდაპირი ლინკის მისაღებად
        invidious_url = f"https://invidious.nerdvpn.de/latest/videos/{video_id}"
